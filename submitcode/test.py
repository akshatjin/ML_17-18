...
def predict(Xtr, Ytr, Xts,k, metric=None):
    ...
    for i in range(Xts.shape[0]):
        ...
        knn = sortedLabel[:k];
        values, counts = np.unique(knn, return_counts=True);  #Gives me the count of each label
        Yts[i] = values[np.argmax(counts)];

    return Yts

def main():
    #We have plugged in the k, we trained the model from model.py on.
    ...
    Ntest= Xts.shape[0];
    #print("shapes od Xtr,Ytr,Xts : ",Xtr.shape,Ytr.shape,Xts.shape);
    #print("For k = ",k);
